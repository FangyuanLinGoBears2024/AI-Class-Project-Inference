...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        # Inference by enumeration and  and by variable elimination only differ by ordering of operations. 
        # I use the same code as the inference by enumeration here. 
        evidenceVariablesSet = set(evidenceDict.keys())
        queryVariablesSet = set(queryVariables)
        currentFactorsList = bayesNet.getAllCPTsWithEvidence(evidenceDict)
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # just don't wanna call total() twice lol
        total = self.total()
        if total > 0:
            for k in self.keys():
                self[k] = self[k] / total
    ...
    def sample(self):
        ...
        for key in dist.keys():
            if cumulative_prob <= u < cumulative_prob + dist[key]:
                return key
            else:
                cumulative_prob += dist[key]
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            probObsGivenPos = self.getObservationProb(observation, pacPos, pos, jailPos)
            # below is the joint prob
            self.beliefs[pos] *= probObsGivenPos
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            transDist = self.getPositionDistribution(gameState, pos)
            for newPos in transDist.keys():
                if newPos in newBeliefs.keys():
                    newBeliefs[newPos] += transDist[newPos] * self.beliefs[pos]
                else: 
                    newBeliefs[newPos] = transDist[newPos] * self.beliefs[pos]
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in self.particles:
            obsProb = self.getObservationProb(observation, pacPosition, particle, jailPosition)
        ...
        if weights.total() == 0:
            ...
            for _ in range(self.numParticles):
                # generate a new position using the sample() method in DiscreteDistribution
                updatedParticles.append(weights.sample())
